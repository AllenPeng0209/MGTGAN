...
def converter(filepath,converter_path):
    ...
    try:
        ...
        multitrack = Multitrack(beat_resolution=16, name=midi_name)
...
def main(music_gerne=None,mode=None):
    """Main function of the converter"""
    print("start coverting ",music_gerne,mode)

    converter_path = os.path.join(ROOT_PATH, music_gerne +'/'+music_gerne+'_'+mode+'/converter')
    cleaner_path = os.path.join(ROOT_PATH, music_gerne +'/'+music_gerne+'_'+mode+'/cleaner')


    midi_paths = get_midi_path(os.path.join(ROOT_PATH, music_gerne +'/'+music_gerne+'_'+mode+'/origin_midi'))
    midi_dict = {}
    ...
    with open(os.path.join(ROOT_PATH, music_gerne +'/'+music_gerne+'_'+mode+'/midis.json'), 'w') as outfile:
        json.dump(midi_dict, outfile)

    print("[Done] {} files out of {} have been successfully converted".format(len(midi_dict), len(midi_paths)))

    with open(os.path.join(ROOT_PATH, music_gerne +'/'+music_gerne+'_'+mode+'/midis.json')) as infile:
    ...
    with open(os.path.join(ROOT_PATH, music_gerne +'/'+music_gerne+'_'+mode+'/midis_clean.json'), 'w') as outfile:
